...
public ArrayList<OurRecipe> selectAllPostPageList(Connection conn, int currentPage, int recordCountPerPage,String type) {
	...
	//글번호의 시작과 끝번호
	 ...
	 String query="";
		...
		case "latest_desc":
			query="SELECT * " + 
			 		"FROM(SELECT ROW_NUMBER()OVER(ORDER BY R.BOARD_NO DESC) AS NUM,R.USER_ID,R.BOARD_NO,R.SUBTITLE,R.TITLE,L.LEVEL_NAME,C.TIME_NAME,F.FILE_PATH,R.LIKE_NUM ,L.LEVEL_CODE,C.TIME_CODE,M.USER_NAME " + 
			 		"FROM RECIPE_BOARD R " + 
		...
		case "like_desc": 
					query="SELECT * FROM(SELECT ROW_NUMBER()OVER(ORDER BY R.BOARD_NO DESC) AS NUM,R.USER_ID,R.BOARD_NO,R.SUBTITLE,R.TITLE,L.LEVEL_NAME,C.TIME_NAME,F.FILE_PATH,R.LIKE_NUM ,L.LEVEL_CODE,C.TIME_CODE,M.USER_NAME " + 
							"    FROM RECIPE_BOARD R\r\n" + 
							"    LEFT JOIN COOK_TIME C ON(C.TIME_CODE=R.TIME_CODE) " + 
							"    LEFT JOIN RECIPE_LEVEL L ON(L.LEVEL_CODE=R.LEVEL_CODE) " + 
							"    LEFT JOIN RECIPE_FILE F ON(F.BOARD_NO=R.BOARD_NO) " + 
		...
		case "level_asc":
					query="SELECT * " + 
							"FROM(SELECT ROW_NUMBER()OVER(ORDER BY R.BOARD_NO DESC) AS NUM,R.USER_ID,R.BOARD_NO,R.SUBTITLE,R.TITLE,L.LEVEL_NAME,C.TIME_NAME,F.FILE_PATH,R.LIKE_NUM,M.USER_NAME " + 
