...
public class SearchDAO {
	...
	public ArrayList<Recipe> selectSearchPostList(Connection conn, int currentPage, int recordCountPerPage,
		...
		case "latest":
			 query =
			" SELECT * " + 
			" FROM (SELECT ROW_NUMBER() OVER(order BY RECIPE_BOARD.BOARD_NO DESC) AS NUM, RECIPE_BOARD.*,recipe_file.file_path ,cook_time.time_name,recipe_level.level_name ,recipe_content.content " + 
			" FROM RECIPE_BOARD  " + 
			" left join recipe_file on (recipe_board.board_no=recipe_file.board_no) " + 
			" left join cook_time on (recipe_board.time_code=cook_time.time_code)  " + 
			" left join recipe_level on (recipe_board.level_Code=recipe_level.level_code)  " + 
			" left join recipe_content on (recipe_board.board_no=recipe_content.board_no) " + 
			" WHERE recipe_BOARD.END_YN='N' AND (RECIPE_BOARD.TITLE like ? or RECIPE_BOARD.SUBTITLE LIKE ? or recipe_content.content like ? ) " + 
		...
		case "like":
			 query =
					 " SELECT * " + 
								" FROM (SELECT ROW_NUMBER() OVER(order BY RECIPE_BOARD.like_num DESC) AS NUM, RECIPE_BOARD.*,recipe_file.file_path ,cook_time.time_name,recipe_level.level_name ,recipe_content.content " + 
								" FROM RECIPE_BOARD  " + 
								" left join recipe_file on (recipe_board.board_no=recipe_file.board_no) " + 
								" left join cook_time on (recipe_board.time_code=cook_time.time_code)  " + 
								" left join recipe_level on (recipe_board.level_Code=recipe_level.level_code)  " + 
