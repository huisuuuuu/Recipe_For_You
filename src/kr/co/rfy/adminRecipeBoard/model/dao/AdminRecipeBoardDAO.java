...
public class AdminRecipeBoardDAO {
	...
	public ArrayList<AdminRecipeBoard> selectAllPostPageList(Connection conn, int currentPage, int recordCountPerPage) {
		...
		String query = "SELECT * " + "FROM(SELECT ROW_NUMBER() OVER(order BY BOARD_NO DESC) AS NUM, rb.*, "
				+ "rc.RECIPE_NAME, m.USER_NAME " + "FROM(RECIPE_BOARD rb JOIN MEMBER m ON(rb.user_id = m.user_id)) "
				+ "JOIN RECIPE_CLASS rc ON(rb.recipe_code = rc.recipe_code) " + "WHERE rb.END_YN='N') "
	...
	public String getPageNavi(Connection conn, int naviCountPerPage, int recordCountPerPage, int currentPage) {
		...
		/*
		 * if((recordTotalCount % recordCountPerPage) > 0) { //나머지가 있다면 페이지를 1개 더 만들어라
		 * pageTotalCount = (recordTotalCount / recordCountPerPage)+1; }else {
		 ...
		 * 
		 * ex1)현재 page가 1이라면 startNavi: 1, endNavi: 5
		 * 
		 * ex2)현재 page가 3이라면 startNavi: 1, endNavi: 5
		 * 
		 * ex3)현재 page가 7이라면 startNavi: 6, endNavi: 10
		 * 
		 * startNavi 공식 startNavi = (((현재 페이지 - 1) / Navi 당 보여질 개수) * Navi 당 보여질 개수) + 1
		 * 
		 * 코드로 표현 startNavi = (((currentPage - 1) / naviCountPerPage) *
		 * naviCountPerPage) + 1
		 * 
		 * endNavi 공식 endNavi = 시작 Navi 값 +(Navi 당 보여질 개수 - 1)
		 * 
		 * 코드로 표현 endNavi = startNavi + (naviCountPerPage - 1)
		 * 
		 */
		...
		if (startNavi == 1) {
			...
			sb.append("<li class='page-item disabled'><span class='page-link'"
					+ "aria-label=\"Previous\"> <span aria-hidden='true'>&laquo;</span>" + "</span></li>");

		} else {
			sb.append("<li class='page-item'><a class='page-link text-dark'"
					+ "href='/recipeBoard/recipeBoardAllSelect.do?currentPage=" + (startNavi - 1)
		...
		for (int i = startNavi; i <= endNavi; i++) {
			if (i == currentPage) {
				sb.append("<li class='page-item active' aria-current='page'>"
						+ "<a class='page-link' href='/recipeBoard/recipeBoardAllSelect.do?currentPage=" + i + "'>" + i
						+ "</a></li>");
			} else {
				sb.append(
						"<li class='page-item'><a class='page-link text-dark' href='/recipeBoard/recipeBoardAllSelect.do?currentPage="
								+ i + "'>" + i + "</a></li>");
			}
		...
		if (endNavi == pageTotalCount) {
			sb.append("<li class='page-item disabled'><span class='page-link'"
					+ "aria-label='Next'> <span aria-hidden='true'>&raquo;</span>" + "</span></li>");
		} else {
			sb.append("<li class='page-item'><a class='page-link text-dark'"
					+ "href='/recipeBoard/recipeBoardAllSelect.do?currentPage=" + (endNavi + 1)
	...
	public AdminRecipeBoard selectOneRecipePost(Connection conn, int boardNo) {
		...
		String query = "SELECT rb.*, rc.*, m.USER_NAME, rl.LEVEL_NAME, ct.TIME_NAME FROM (((RECIPE_BOARD rb JOIN RECIPE_CLASS rc "
				...
				+ "JOIN COOK_TIME ct ON (rb.time_code = ct.time_code) " + "WHERE rb.BOARD_NO=? AND rb.END_YN='N'";
	...
	// recipe_file에서 해당 boardNo을 가진 recipe의 이미지 목록 가져오기
	public ArrayList<RecipeImage> selectRecipePostImageList(Connection conn, String [] recipeBoardNoValues) {
		...
		String query = "SELECT * FROM RECIPE_FILE WHERE END_YN='N' AND BOARD_NO IN("+values+")";
	...
	// recipe_file에서 해당 boardNo을 가진 recipe의 컨텐츠 목록 가져오기
	public ArrayList<RecipeContent> selectRecipePostContentList(Connection conn, String [] recipeBoardNoValues) {
		...
		String query = "SELECT * FROM RECIPE_CONTENT WHERE END_YN='N' AND BOARD_NO IN("+values+")";
	...
	// recipe_file에서 해당 boardNo을 가진 recipe의 재료 목록 가져오기
	public ArrayList<RecipeIngredient> selectRecipePostIngredientList(Connection conn, String [] recipeBoardNoValues) {
		...
		String query = "SELECT * FROM RECIPE_MANAGEMENT WHERE END_YN='N' AND BOARD_NO IN("+values+")";
	...
	public ArrayList<AdminRecipeBoard> selectSearchPostList(Connection conn, int currentPage, int recordCountPerPage,
		...
		case "writer":
			query = "SELECT * " + "FROM(SELECT ROW_NUMBER() OVER(order BY BOARD_NO DESC) AS NUM, rb.*, "
					+ "rc.RECIPE_NAME, m.USER_NAME " + "FROM(RECIPE_BOARD rb JOIN MEMBER m ON(rb.user_id = m.user_id)) "
					+ "JOIN RECIPE_CLASS rc ON(rb.recipe_code = rc.recipe_code) "
					+ "WHERE rb.END_YN='N' AND m.user_name like ?) " + "WHERE NUM BETWEEN ? AND ?";
			break;
		case "subject":
			query = "SELECT * " + "FROM(SELECT ROW_NUMBER() OVER(order BY BOARD_NO DESC) AS NUM, rb.*, "
					+ "rc.RECIPE_NAME, m.USER_NAME " + "FROM(RECIPE_BOARD rb JOIN MEMBER m ON(rb.user_id = m.user_id)) "
					+ "JOIN RECIPE_CLASS rc ON(rb.recipe_code = rc.recipe_code) "
					+ "WHERE rb.END_YN='N' AND rb.title like ?) " + "WHERE NUM BETWEEN ? AND ?";
			break;
	...
	public String getSearchPageNavi(Connection conn, int naviCountPerPage, int recordCountPerPage, int currentPage,
		...
		if (startNavi == 1) {
			...
			sb.append("<li class='page-item'><a class='page-link text-dark'"
					+ "href='/recipeBoard/recipeBoardPostSearch.do?currentPage=" + (startNavi - 1) + "&keyword="
					+ keyword + "&type=" + type + "' aria-label='Previous'> <span aria-hidden='true'>&laquo;</span>"
					+ "</a></li>");
		}
		...
		for (int i = startNavi; i <= endNavi; i++) {
			if (i == currentPage) {
				sb.append("<li class='page-item active' aria-current='page'>"
						+ "<a class='page-link' href='/recipeBoard/recipeBoardPostSearch.do?currentPage=" + i
						+ "&keyword=" + keyword + "&type=" + type + "'>" + i + "</a></li>");
			} else {
				sb.append(
						"<li class='page-item'><a class='page-link text-dark' href='/recipeBoard/recipeBoardPostSearch.do?currentPage="
								+ i + "&keyword=" + keyword + "&type=" + type + "'>" + i + "</a></li>");
			}
		...
		if (endNavi == pageTotalCount) {
			...
			sb.append("<li class='page-item'><a class='page-link text-dark'"
					+ "href='/recipeBoard/recipeBoardPostSearch.do?currentPage=" + (endNavi + 1) + "&keyword=" + keyword
					+ "&type=" + type + "' aria-label=\"Next\"> <span aria-hidden='true'>&raquo;</span>" + "</a></li>");
		}
	...
	private int totalSearchCount(Connection conn, String keyword, String type) {
		...
		case "writer":
			...
			query = "SELECT count(*) as count " + "FROM(RECIPE_BOARD rb JOIN MEMBER m ON(rb.user_id = m.user_id)) "
					+ "JOIN RECIPE_CLASS rc ON(rb.recipe_code = rc.recipe_code) "
	...
	private int totalCategoryCount(Connection conn, String recipeCode) {

		PreparedStatement pstmt = null;
		ResultSet rset = null;

		int count = 0;
		String query = "SELECT count(*) as count FROM RECIPE_BOARD WHERE RECIPE_CODE=? AND END_YN='N'";
	...
	public int deletePostImage(Connection conn, int boardNo) {
	...
	public int deletePostIngredient(Connection conn, int boardNo) {
	...
	public int deletePostContent(Connection conn, int boardNo) {
	...
	public int deleteAdminBoardList(Connection conn, String[] recipeBoardNoValues) {
	...
	public int deleteRecipeContentList(Connection conn, String[] recipeBoardNoValues) {
		...
		// [0] = 100 / [1] = 101 / [2] 102
		// 100,101,102
		String query = "UPDATE RECIPE_CONTENT SET END_YN='Y' WHERE BOARD_NO IN(" + values + ")";
	...
	public int deleteRecipeImageList(Connection conn, String[] recipeBoardNoValues) {
		...
		// [0] = 100 / [1] = 101 / [2] 102
		// 100,101,102
		String query = "UPDATE RECIPE_FILE SET END_YN='Y' WHERE BOARD_NO IN(" + values + ")";
	...
	public int deleteRecipeIngredientList(Connection conn, String[] recipeBoardNoValues) {
		...
		// [0] = 100 / [1] = 101 / [2] 102
		// 100,101,102
		String query = "UPDATE RECIPE_MANAGEMENT SET END_YN='Y' WHERE BOARD_NO IN(" + values + ")";
	...
	public int insertRecipePostIngredient(Connection conn, int boardNo, String[] ingredientNameValues,
		...
		for (int i = 0; i < ingredientNameValues.length; i++) {
			...
			query.append(" ");
			query.append("INTO RECIPE_MANAGEMENT VALUES(" + boardNo + ", '" + ingredientNameValues[i] + "', '"
					+ ingredientNum[i] + "', 'N')");
			query.append(" ");
		}
	...
	public String getCategoryPageNavi(Connection conn, int naviCountPerPage, int recordCountPerPage, int currentPage,
			String recipeCode) {

		int recordTotalCount = totalCategoryCount(conn, recipeCode); // 검색된 글의 전체 개수
		...
		if (startNavi == 1) {
			...
			sb.append("<li class='page-item'><a class='page-link text-dark'"
					+ "href='/recipeBoard/recipeBoardCategorySelect.do?currentPage=" + (startNavi - 1) + "&recipeCode="
					+ recipeCode + "' aria-label='Previous'> <span aria-hidden='true'>&laquo;</span>" + "</a></li>");
		}

		for (int i = startNavi; i <= endNavi; i++) {
			if (i == currentPage) {
				sb.append("<li class='page-item active' aria-current='page'>"
						+ "<a class='page-link' href='/recipeBoard/recipeBoardCategorySelect.do?currentPage=" + i
						+ "&recipeCode=" + recipeCode + "'>" + i + "</a></li>");
			} else {
				sb.append(
						"<li class='page-item'><a class='page-link text-dark' href='/recipeBoard/recipeBoardCategorySelect.do?currentPage="
								+ i + "&recipeCode=" + recipeCode + "'>" + i + "</a></li>");
			}
		...
		if (endNavi == pageTotalCount) {
			...
			sb.append("<li class='page-item'><a class='page-link text-dark'"
					+ "href='/recipeBoard/recipeBoardCategorySelect.do?currentPage=" + (endNavi + 1) + "&recipeCode="
					+ recipeCode + "' aria-label=\"Next\"> <span aria-hidden='true'>&raquo;</span>" + "</a></li>");
		}
	...
	public ArrayList<AdminRecipeBoard> selectPostCategoryList(Connection conn, int currentPage, int recordCountPerPage,
			String recipeCode) {
		...
		String query = "SELECT * " + "FROM(SELECT ROW_NUMBER() OVER(order BY BOARD_NO DESC) AS NUM, rb.*, "
				+ "rc.RECIPE_NAME, m.USER_NAME " + "FROM(RECIPE_BOARD rb JOIN MEMBER m ON(rb.user_id = m.user_id)) "
				+ "JOIN RECIPE_CLASS rc ON(rb.recipe_code = rc.recipe_code) "
				+ "WHERE rb.END_YN='N' AND rb.recipe_code=?) " + "WHERE NUM BETWEEN ? AND ?";
