...
<form action="/admin/MemberSearchBlack.do" method="get" id="search">
...
<%if(!list.isEmpty()) { %>
	<table>
		<tr>
			<th>회원번호
			<a href="/admin/blackList.do" class="UpDown"><i class="fas fa-angle-down"></i></a>
...
<tbody>
		...
		<%}else { %>
			...
			<%} %>
				...
				<td><%=m.getUser_Id() %></td>
				<%if(m.getEnd_YN()=='N'){%>
				<td><a class="link" href="/admin/memberSelectBlack.do?userNo=<%=m.getUser_No() %>&currentPage=<%=currentPage%>" ><%=m.getUser_Name() %></a></td>
				<%}else{ %>
