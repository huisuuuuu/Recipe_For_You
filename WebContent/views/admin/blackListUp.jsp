...
<h2 class="tit">블랙 리스트 관리</h2>

<form action="/admin/MemberSearchBlackUP.do" method="get" id="search">
...
<%if(!list.isEmpty()) { %>
	...
	<thead>
		<tr>
			<th>
			회원번호
		<a href="/admin/blackList.do" class="UpDown"><i class="fas fa-angle-down" style="color:black;"></i></a>
		<a href="/admin/blackListUp.do" class="UpDown"><i class="fas fa-angle-up"></i></a>
		...
		<%}else { %>
			...
			<%} %>
				...
				<%if(m.getEnd_YN()=='N'){ %>
				<td><a class="link" href="/admin/memberSelectBlackUp.do?userNo=<%=m.getUser_No() %>&currentPage=<%=currentPage%>" ><%=m.getUser_Name() %></a></td>
				<%}else{ %>
